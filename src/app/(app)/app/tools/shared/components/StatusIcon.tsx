import type { AuditCheckStatus } from '@/types/audit'

interface StatusIconProps {
  status: AuditCheckStatus
  /** 'sm' = w-4 h-4, 'md' = w-5 h-5 (default) */
  size?: 'sm' | 'md'
}
...
export function StatusIcon({ status, size = 'md' }: StatusIconProps) {
  ...
  const containerCls = size === 'sm' ? 'w-4 h-4' : 'w-5 h-5'
  const iconCls = size === 'sm' ? 'w-2.5 h-2.5' : 'w-3 h-3'

  return (
    <div
      className={`${containerCls} rounded-full ${bg} border flex items-center justify-center shrink-0`}
    >
      <svg viewBox="0 0 20 20" fill="currentColor" className={`${iconCls} ${text}`}>
