...
export default function GA4AuditApp() {
  ...
  const [viewingHistory, setViewingHistory] = useState(false) // true = show history list
  ...
  // ── Save report ──────────────────────────────────────────────────────
  const handleSaveReport = useCallback(async (reportToSave: AuditReport) => {
    setSaving(true)
    try {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-border bg-card/30 px-6 md:px-8 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {status === 'ready' && savedReports.length > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors cursor-pointer ${
                viewingHistory
                  ...
                  : 'border-border text-muted-foreground hover:text-foreground hover:border-orange-500/30'
              }`}
            >
              ...
              Past Audits
              <span className="bg-muted/60 text-muted-foreground px-1.5 py-0.5 rounded-md text-[10px] font-bold">
                {savedReports.length}
              </span>
            </button>
          )}
                ...
                {[
                  {
                    ...
                    desc: 'If you signed in with Google, your account is already connected. Otherwise, you\'ll be asked to connect your Google account for read-only analytics access.',
                  },
                  {
                    step: '2',
              ...
              <div className="rounded-2xl border border-border bg-card/80 backdrop-blur-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-muted-foreground">{savedReports.length} report{savedReports.length !== 1 ? 's' : ''}</span>
                  </div>
                </div>

                {savedReportsLoading ? (
                  <div className="p-8 text-center">
                    <svg className="w-5 h-5 text-muted-foreground animate-spin mx-auto" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeOpacity="0.15" />
                      <path d="M12 2a10 10 0 019.95 9" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
                    </svg>
                    <p className="text-xs text-muted-foreground mt-2">Loading reports...</p>
                  </div>
                ) : savedReports.length === 0 ? (
                  <div className="p-8 text-center">
                    <p className="text-sm text-muted-foreground">No saved reports yet.</p>
                    <p className="text-xs text-muted-foreground mt-1">Reports are automatically saved after each audit.</p>
                  </div>
                ) : (
                  <div className="divide-y divide-border/50 max-h-[50vh] overflow-y-auto">
                    {savedReports.map((r) => {
                      const scoreColor =
                        r.healthScore >= 80 ? 'text-emerald-500' : r.healthScore >= 50 ? 'text-amber-500' : 'text-red-500'
                      const scoreBg =
                        r.healthScore >= 80 ? 'bg-emerald-500/10 border-emerald-500/20' : r.healthScore >= 50 ? 'bg-amber-500/10 border-amber-500/20' : 'bg-red-500/10 border-red-500/20'
                      return (
                        <div
                          key={r.id}
                          className="px-5 py-3.5 hover:bg-muted/5 transition-colors flex items-center justify-between gap-4 group"
                        >
                          <button
                            onClick={() => handleLoadReport(r.id)}
                            className="flex items-center gap-3.5 min-w-0 flex-1 text-left cursor-pointer"
                          >
                            <div className={`w-10 h-10 rounded-lg ${scoreBg} border flex items-center justify-center shrink-0`}>
                              <span className={`text-sm font-bold ${scoreColor}`}>{r.healthScore}</span>
                            </div>
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                {r.summary.passed > 0 && (
                                  <span className="text-[10px] text-emerald-500 bg-emerald-500/10 rounded px-1 py-0.5">{r.summary.passed} passed</span>
                                )}
                                {r.summary.warnings > 0 && (
                                  <span className="text-[10px] text-amber-500 bg-amber-500/10 rounded px-1 py-0.5">{r.summary.warnings} warn</span>
                                )}
                                {r.summary.failures > 0 && (
                                  <span className="text-[10px] text-red-500 bg-red-500/10 rounded px-1 py-0.5">{r.summary.failures} failed</span>
                                )}
                              </div>
                            </div>
                          </button>
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-500/10 text-muted-foreground hover:text-red-500 transition-all cursor-pointer shrink-0"
                            title="Delete report"
                          >
                            <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                              <path fillRule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.519.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 01.78.72l.5 6a.75.75 0 01-1.5.12l-.5-6a.75.75 0 01.72-.78zm3.62.72a.75.75 0 10-1.5-.06l-.5 6a.75.75 0 101.5.06l.5-6z" clipRule="evenodd" />
                            </svg>
                          </button>
                        </div>
                      )
            ...
            {connectPhase === 'popup' && (
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1.5 max-w-xs mx-auto">
                    A pop-up window has opened. Sign in with Google and grant read-only analytics access to continue.
                  </p>
                </div>
        ...
        {status === 'selecting' && (
          <div className="px-6 md:px-8 py-8 max-w-6xl mx-auto space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {properties.length} propert{properties.length === 1 ? 'y' : 'ies'} found on your account.
                </p>
              </div>
              {properties.length > 4 && (
                <div className="relative w-full sm:w-56">
                  <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-muted-foreground absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
              ...
              {propertySearch && properties.filter((p) =>
                p.displayName.toLowerCase().includes(propertySearch.toLowerCase()) ||
                p.propertyId.toLowerCase().includes(propertySearch.toLowerCase()),
              ).length === 0 && (
          ...
          <div className="px-6 md:px-8 py-6 space-y-5 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3.5 text-center">
                ...
                <p className="text-[11px] text-emerald-600/70 dark:text-emerald-400/70 font-medium mt-0.5">Passed</p>
              </div>
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3.5 text-center">
                ...
                <p className="text-[11px] text-amber-600/70 dark:text-amber-400/70 font-medium mt-0.5">Warnings</p>
              </div>
              <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-3.5 text-center">
                ...
                <p className="text-[11px] text-red-600/70 dark:text-red-400/70 font-medium mt-0.5">Failed</p>
              </div>
              <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3.5 text-center">
                <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{report.summary.info}</p>
                <p className="text-[11px] text-blue-600/70 dark:text-blue-400/70 font-medium mt-0.5">Info</p>
              </div>
            </div>
                    ...
                    >
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="flex h-1 rounded-full overflow-hidden bg-muted/30 max-w-xs">
                          {passCount > 0 && (
                            <div className="bg-emerald-500" style={{ width: `${(passCount / checks.length) * 100}%` }} />
                          )}
                          {infoCount > 0 && (
                            <div className="bg-blue-500" style={{ width: `${(infoCount / checks.length) * 100}%` }} />
                          )}
                          {warnCount > 0 && (
                            <div className="bg-amber-500" style={{ width: `${(warnCount / checks.length) * 100}%` }} />
                          )}
                          {failCount > 0 && (
                            <div className="bg-red-500" style={{ width: `${(failCount / checks.length) * 100}%` }} />
                          )}
                        </div>
                      </div>
                      <svg
                        ...
                        {checks.map((check, idx) => (
                          <div
                            ...
                            className={`p-4 md:px-5 flex gap-3 ${idx < checks.length - 1 ? 'border-b border-border/50' : ''} ${
                              check.status === 'fail' ? 'bg-red-500/[0.02]' : check.status === 'warn' ? 'bg-amber-500/[0.02]' : ''
                            }`}
                          >
            ...
            <div className="text-center space-y-1.5">
              <p className="text-[11px] text-muted-foreground">
                Audit completed on {new Date(report.timestamp).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} at {new Date(report.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
              </p>
              {saving && (
